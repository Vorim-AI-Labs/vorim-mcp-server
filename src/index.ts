...
function resolveBaseUrl(): string {
  ...
  if (!ok) {
    console.error(
      `[vorim-mcp] Refusing to use VORIM_BASE_URL=${raw}: must be https:// ` +
...
server.registerTool(
  ...
  async ({ agent_id, scope, valid_until, rate_limit_max, rate_limit_window }) => {
    ...
    if (hasMax !== hasWindow) {
      throw new Error(
        "rate_limit_max and rate_limit_window must be provided together (got only one).",
      );
    }
    if (hasMax && hasWindow) {
...
server.registerTool(
  ...
  async ({ agent_id, event_type, action, result, resource, permission, latency_ms, error_code }) => {
    ...
    if (latency_ms !== undefined) event.latency_ms = latency_ms;
...
server.registerTool(
  ...
  async ({ connection_id, agent_id, scopes_delegated, max_requests_per_hr, valid_until }) => {
    ...
    if (max_requests_per_hr !== undefined) body.max_requests_per_hr = max_requests_per_hr;
    if (valid_until) body.valid_until = valid_until;
...
async function vorimPublicPost(path: string, body: unknown): Promise<{ ok: boolean; status: number; json: Record<string, unknown> }> {
  const response = await fetch(`${BASE_URL}/v1${path}`, {
    method: "POST",
  ...
  let json: Record<string, unknown> = {};
  try { json = (await response.json()) as Record<string, unknown>; } catch { /* non-JSON */ }
...
server.registerTool(
  ...
  {
    description:
      "Start onboarding a user who does NOT yet have a Vorim API key (device-authorization flow). " +
  ...
  async ({ client_name }) => {
    ...
    if (!ok) throw new Error(`Could not start onboarding (HTTP ${status})`);
    ...
    return text({
      ...
      instructions:
        `Show the human this EXACTLY: "Approve this agent at ${d.verification_uri} and enter code ${d.user_code}". ` +
...
server.registerTool(
  ...
  {
    description:
      "Check whether the human has approved the onboarding request from vorim_onboard_start. " +
  ...
  async ({ device_code }) => {
    ...
    if (ok && (json.data as Record<string, unknown> | undefined)?.api_key) {
      const d = json.data as Record<string, unknown>;
      return text({
        status: "approved",
        api_key: d.api_key,
    ...
    const code = ((json.error as Record<string, unknown> | undefined)?.code as string) || "authorization_pending";
